...
const Statistiques_Achats = () => {
  ...
  useEffect(() => {
    ...
    ipcRenderer.on("AchatsSending", (e, result) => {
      let NomsDesProduits = [];
      let Produits = [];
      let quantites = [];
      ...
      setNomsDesProduits(NomsDesProduits.slice(0,10));
      ...
      setquantiteList(quantites.slice(0,10));
    });
  }, []);
  const options = {
    chart: {
  ...
  const options2 = {
    ...
    xaxis: {
      categories: ["","Jan","Ferv","Mar","Avr","Mai","Juin","Juil","Aou","Sep","Oct","Nov","Dec"],
    },
    stroke: {
      curve: 'smooth',
    }
  ...
  return (
    <section className="statistiques_section">
      <h1 className="statistiques_page_title">statistiques Des achats</h1>
...
export default Statistiques_Achats;
